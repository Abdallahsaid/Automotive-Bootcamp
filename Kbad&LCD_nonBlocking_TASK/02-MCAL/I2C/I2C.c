...
void I2C_send_slave_address_with_write_req(u8_t slave_address)
{
...
void I2C_send_slave_address_with_read_req(u8_t slave_address)
{
...
void I2C_slave_write_byte(u8_t byte)
{
	TWDR = byte;
	TWCR = (1<<TWINT) | (1<<TWEA) | (1<<TWEN);         // Clear TWI interrupt flag,Get enable acknowledgment, Enable TWI
	while((GET_BIT(TWCR,TWINT)) == 0 );                     // Wait for TWINT flag
